...
CFLAGS+= -m32 -Wall -O3 -fomit-frame-pointer \
...
ifeq ($(shell uname -s), Darwin)
   ...
   LIBS=/opt/local/lib/libelf.a

# To use for prod:
#   LIBS=-L/usr/local/lib -L. -ldl -lelf -lpthread
#   LIBS=-L/usr/local/ppc/lib -L. -ldl -lelf -pthread
else
...
ifeq ($(shell uname -o), Cygwin)
   OSNAME=Windows
   CFLAGS+=-I/usr/local/include -I/usr/local/include/libelf -I/usr/include/winpcap -DCYGWIN \
	-D_FILE_OFFSET_BITS=64
   LIBS=-L/usr/local/lib -L. -lelf -lpthread -luuid
...
HDR=mempool.h registry.h rbtree.h hash.h utils.h parser.h plugin.h \
	...
	vm.h cpu.h tcb.h jit_op.h memory.h device.h \
...
SOURCES=mempool.c registry.c rbtree.c hash.c sbox.c utils.c parser.c \
	...
	dynamips.c insn_lookup.c vm.c cpu.c tcb.c jit_op.c \
...
HDR += mips64_ppc32_trans.h ppc32_ppc32_trans.h
SOURCES += mips64_ppc32_trans.c ppc32_ppc32_trans.c
CFLAGS += -m32
endif
...
FILE_LIST := $(HDR) $(SOURCES) $(SUPPL) \
	x86-codegen.h amd64-codegen.h ppc-codegen.h \
	...
	mips64_ppc32_trans.c mips64_ppc32_trans.h \
