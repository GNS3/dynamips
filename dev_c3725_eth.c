                                 ...
                                 const struct cisco_eeprom *eeprom)
{
   ...
   for(i=0;i<data->nr_port;i++) {
      data->port[i] = dev_am79c971_init(vm,card->dev_name,interface_type,
                                        ...
                                        c3725_net_irq_for_slot_port(slot,i));
   }
...
static int dev_c3725_nm_eth_shutdown(vm_instance_t *vm,struct cisco_card *card)
{
...
static int dev_c3725_gt96100_fe_init(vm_instance_t *vm,struct cisco_card *card)
{
   if (card->slot_id != 0) {
...
/* Bind a Network IO descriptor */
static int
dev_c3725_gt96100_fe_set_nio(vm_instance_t *vm,struct cisco_card *card,
                             u_int port_id,netio_desc_t *nio)
{
