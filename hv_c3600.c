...
/* Create a C3600 instance */
static int cmd_create(hypervisor_conn_t *conn,int argc,char *argv[])
{
   c3600_t *router;

   if (!(router = c3600_create_instance(argv[0],atoi(argv[1])))) {
      hypervisor_send_reply(conn,HSC_ERR_CREATE,1,
                            "unable to create C3600 instance '%s'",
                            argv[0]);
      return(-1);
   }
   ...
   hypervisor_send_reply(conn,HSC_INFO_OK,1,"C3600 '%s' created",argv[0]);
   return(0);
}

/* Delete a C3600 instance */
static int cmd_delete(hypervisor_conn_t *conn,int argc,char *argv[])
{
   int res;

   res = c3600_delete_instance(argv[0]);

   if (res == 1) {
      hypervisor_send_reply(conn,HSC_INFO_OK,1,"C3600 '%s' deleted",argv[0]);
   } else {
      hypervisor_send_reply(conn,HSC_ERR_DELETE,1,
                            "unable to delete C3600 '%s'",argv[0]);
   }
...
static int cmd_set_chassis(hypervisor_conn_t *conn,int argc,char *argv[])
{
   ...
   if (!(vm = hypervisor_find_vm(conn,argv[0],VM_TYPE_C3600)))
      return(-1);
...
static int cmd_set_iomem(hypervisor_conn_t *conn,int argc,char *argv[])
{
   ...
   VM_C3600(vm)->nm_iomem_size = 0x8000 | atoi(optarg);
...
static int cmd_start(hypervisor_conn_t *conn,int argc,char *argv[])
{
   ...
   if (c3600_init_instance(router) == -1) {
      vm_release(vm);
   ...
   vm_release(vm);
   hypervisor_send_reply(conn,HSC_INFO_OK,1,"C3600 '%s' started",argv[0]);
   return(0);
}

/* Stop a C3600 instance */
static int cmd_stop(hypervisor_conn_t *conn,int argc,char *argv[])
{
   ...
   if (c3600_stop_instance(router) == -1) {
      vm_release(vm);
   ...
   vm_release(vm);
   hypervisor_send_reply(conn,HSC_INFO_OK,1,"C3600 '%s' stopped",argv[0]);
   return(0);
}
...
static int cmd_nm_bindings(hypervisor_conn_t *conn,int argc,char *argv[])
{
   ...
   for(i=0;i<C3600_MAX_NM_BAYS;i++) {
      nm_type = c3600_nm_get_type(router,i);
      if (nm_type)
...
{   
   ...
   if (c3600_nm_add_binding(router,argv[2],nm_bay) == -1) {
      vm_release(vm);
      hypervisor_send_reply(conn,HSC_ERR_BINDING,1,
                            "C3600 %s: unable to add NM binding for slot %u",
                            argv[0],nm_bay);
      return(-1);
   }
...
static int cmd_remove_nm_binding(hypervisor_conn_t *conn,int argc,char *argv[])
{
   ...
   if (c3600_nm_remove_binding(router,nm_bay) == -1) {
      vm_release(vm);
      hypervisor_send_reply(conn,HSC_ERR_BINDING,1,
                            "C3600 %s: unable to remove NM binding for "
...
{  
   ...
   if (c3600_nm_add_nio_binding(router,nm_bay,port_id,argv[3]) == -1) {
      vm_release(vm);
      hypervisor_send_reply(conn,HSC_ERR_BINDING,1,
                            "C3600 %s: unable to add NIO binding for "
                                  ...
                                  int argc,char *argv[])
{
   ...
   if (c3600_nm_remove_nio_binding(router,nm_bay,port_id) == -1) {
      vm_release(vm);
      hypervisor_send_reply(conn,HSC_ERR_BINDING,1,
                            "C3600 %s: unable to remove NIO binding for "
                            "interface %u/%u",argv[0],nm_bay,port_id);
      return(-1);
   }
...
{  
   ...
   if (c3600_nm_enable_nio(router,nm_bay,port_id) == -1) {
      vm_release(vm);
      hypervisor_send_reply(conn,HSC_ERR_BINDING,1,
                            "C3600 %s: unable to enable NIO for "
                            "interface %u/%u",argv[0],nm_bay,port_id);
      return(-1);
   }
...
{  
   ...
   if (c3600_nm_disable_nio(router,nm_bay,port_id) == -1) {
      vm_release(vm);
      hypervisor_send_reply(conn,HSC_ERR_BINDING,1,
                            "C3600 %s: unable to unset NIO for "
                            "interface %u/%u",
...
int hypervisor_c3600_init(void)
{
   hypervisor_module_t *module;

   module = hypervisor_register_module("c3600");
   assert(module != NULL);
