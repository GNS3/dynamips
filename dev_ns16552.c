 ...
 * NS16552 DUART.
 *
 * Changes August 2010 Gordon Russell
 *   Improved the understanding of some of the registered, and activated the AUX port
 */
   ...
   switch(offset) {
      ...
      case 0x08:
         if (d->div_latch == 0) {
           ...
           //cpu_log(cpu,"NS16552","Modem Control LSB Baud\n");
           if (op_type == MTS_WRITE) {
      ...
      case 0x09:
         if (d->div_latch == 0) {
           ...
           //cpu_log(cpu,"NS16552","Modem Control MSB Baud\n");
           if (op_type == MTS_WRITE) {
      ...
      case 0x0A:
         if (d->div_latch == 0) {
           ...
           cpu_log(cpu,"NS16552","Modem Control Alternate function\n");
           //if (op_type == MTS_WRITE) {
           //} else {
           //}
         }
      ...
      case 0x0B:
         if (op_type == MTS_READ) {
           *data = d->line_control_reg;
           cpu_log(cpu,"NS16552","read from Line Control Reg channel %d, value=0x%llx\n",
                   channel,*data);
         } else {
           d->line_control_reg = (uint)*data;
           //cpu_log(cpu,"NS16552","write to Line Control Reg channel %d, value=0x%llx\n",
           //        channel,*data);
           uint bits = 5;
           char *stop = "1";
           char *parity = "no ";
           ...
           if (*data & LCR_PARITYEV)
             parityeven="even";
           cpu_log(cpu,"NS16552","LCR: %d bits, %s stopbits, %sparity(%s)\n",bits,stop,parity,parityeven);
           if (*data & LCR_STICKP)
             cpu_log(cpu,"NS16552","Line Control Reg Stick Parity\n");
           if (*data & LCR_SETBREAK)
             cpu_log(cpu,"NS16552","Line Control Reg SETBREAK\n");

           // DIV LATCH changes the behavior of 0x0,0x1,and 0x2
           if (*data & LCR_DIVLATCH) {
             d->div_latch = 1;
             cpu_log(cpu,"NS16552","Line Control Reg Access DIV LATCH\n");
           } else {
             uint baud;
             d->div_latch = 0;
             ...
             if (d->baud_divisor > 0) {
               ...
               cpu_log(cpu,"NS16552","Line Control Reg baud is %d bps\n",baud);
             } else {
               baud = 0;
             }
           }
      ...
      case 0x0C:
         if (op_type == MTS_READ) {
           //*data = d->line_control_reg;
           cpu_log(cpu,"NS16552","read from Modem Control Reg channel %d, value=0x%llx\n",
                   channel,*data);
         } else {
           //d->line_control_reg = (uint)*data;
           char *f1 = ""; char *f2 = ""; char *f3 = ""; char *f4 = ""; char *f5 = "";
           //cpu_log(cpu,"NS16552","write to Modem Control Reg channel %d, value=0x%llx\n",
           //        channel,*data);
           ...
           cpu_log(cpu,"NS16552","Modem Control Reg: %s%s%s%s%s\n",f1,f2,f3,f4,f5);
         }
         break;
      /* Line Status Register (LSR) */
      case 0x05:
