...
static int c7200_nvram_push_config(vm_instance_t *vm,u_char *buffer,size_t len)
{
   ...
   ret = generic_nvram_push_config(vm, "nvram", vm->nvram_size*1024, vm->nvram_rom_space, 0, VM_C7200(vm)->npe_driver->nvram_addr + vm->nvram_rom_space, FS_NVRAM_FORMAT_ABSOLUTE, buffer, len, NULL, 0);
